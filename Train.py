...
class DAF2D(DAF3D):
    def __init__(self, body):
        ...
        self.fuse1 = nn.Sequential(
            nn.Conv2d(512, 64, kernel_size=1), nn.GroupNorm(32, 64), nn.PReLU(),
            nn.Conv2d(64, 64, kernel_size=3, padding=1), nn.GroupNorm(32, 64), nn.PReLU(),
            nn.Conv2d(64, 64, kernel_size=3, padding=1), nn.GroupNorm(32, 64), nn.PReLU()
        )
        self.attention4 = nn.Sequential(
            nn.Conv2d(192, 64, kernel_size=1), nn.GroupNorm(32, 64), nn.PReLU(),
            nn.Conv2d(64, 64, kernel_size=3, padding=1), nn.GroupNorm(32, 64), nn.PReLU(),
            nn.Conv2d(64, 64, kernel_size=3, padding=1), nn.Sigmoid()
        )
        ...
        self.refine = nn.Sequential(nn.Conv2d(256, 64, kernel_size=1),
                                    nn.GroupNorm(32, 64),
                                    nn.PReLU(),)
    ...
    def forward(self, x):
        ...
        predict1_1 = F.interpolate(predict1_1, size=x.size()[2:], mode='bilinear')
        predict1_2 = F.interpolate(predict1_2, size=x.size()[2:], mode='bilinear')
        predict1_3 = F.interpolate(predict1_3, size=x.size()[2:], mode='bilinear')
        predict1_4 = F.interpolate(predict1_4, size=x.size()[2:], mode='bilinear')

        predict2_1 = F.interpolate(predict2_1, size=x.size()[2:], mode='bilinear')
        predict2_2 = F.interpolate(predict2_2, size=x.size()[2:], mode='bilinear')
        predict2_3 = F.interpolate(predict2_3, size=x.size()[2:], mode='bilinear')
        predict2_4 = F.interpolate(predict2_4, size=x.size()[2:], mode='bilinear')

        predict = F.interpolate(predict_undersample, size=x.size()[2:], mode='bilinear')
...
def ps_learner(data: DataBunch, base_arch: Callable, cut: Union[int, Callable] = None, pretrained: bool = True,
    ...
    if pretrained: learn.freeze_backbone()

    if init:
        for g in learn.layer_groups[1:]:
            chd = children(g)
            for l in chd:
                if isinstance(l, ASPP_module_2d) or isinstance(l, ASPP_module) \
                        or isinstance(l, nn.GroupNorm) or isinstance(l, nn.PReLU):
                    continue
                apply_init(l, init)
    return learn
...
class psKernel():
    def __init__(self, stage='dev'):
        ...
        self.noise_th = 75.0 * (self.sz / 128.0) ** 2  # threshold for the number of predicted pixels
        self.best_thr0 = 0.2  # preliminary value of the threshold for metric calculation
        self.best_thr0_sigmoid = 0.6  # preliminary value of the threshold for metric calculation
    ...
    def _get_fold_data(self, fold):
        ...
        valid_idx = list(kf.split(list(range(len(Path(self.TRAIN).ls())))))[fold][1]
        # Create databunch
        data = (SegmentationItemList.from_folder(self.TRAIN)
    ...
    def _dice(self, input: Tensor, targs: Tensor, thr: float = None, iou: bool = False, eps: float = 1e-8) -> Rank0Tensor:
        n = targs.shape[0]
        input = input[-1]
...
class ps_multi_Loss(nn.Module):
    ...
    def forward(self, input, target, **kwargs):
        #return predict1_1, predict1_2, predict1_3, predict1_4, \
        #       predict2_1, predict2_2, predict2_3, predict2_4, predict
        outputs1, outputs2, outputs3, outputs4, outputs1_1, outputs1_2, outputs1_3, outputs1_4, output = input

        label = target.float()
        label = label.clamp(self.eps, 1. - self.eps)
...
if __name__ == '__main__':
    ...
    learner = ps_learner(data, torchvision.models.resnext50_32x4d, cut=8,  # only used up to 7
                         #loss_func=ps_multi_Loss(),
                         metrics=[partial(k._dice, thr=0.5),partial(k._dice, thr=0.6),partial(k._dice, thr=0.7), partial(k._dice, thr=0.8),partial(k._dice, thr=0.9)], loss_func=ps_multi_Loss(),
